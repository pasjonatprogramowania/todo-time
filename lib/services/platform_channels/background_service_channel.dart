...
class BackgroundServiceChannel {
  ...
  // Event Channel for Screen Time Updates
  static const _screenTimeEventChannel = EventChannel('com.tasktime.app/screen_time_events');
  static Stream<int>? _screenTimeStream; // Stream of remaining milliseconds

  static Stream<int> get screenTimeUpdateStream {
    _screenTimeStream ??= _screenTimeEventChannel
        .receiveBroadcastStream()
        .map((event) => event is int ? event : (event is num ? event.toInt() : 0)); // Ensure int
    return _screenTimeStream!;
  }
}
