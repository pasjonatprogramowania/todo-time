...
import 'package:task_time/presentation/providers/screen_time_provider.dart'; // Import screen time provider
              ...
              IconButton(
                ...
                onPressed: () async {
                  try {
                    ...
                    // If task is marked as complete (was not completed before)
                    if (!wasCompleted) {
                      // The use case itself doesn't return the updated task.
                      // We assume the stream provider for tasks will update the UI.
                      // Now, add the awarded time.
                      await ref.read(earnedScreenTimeRepositoryProvider).addScreenTime(task.timeAward);
                       ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('Task completed! +${task.timeAward.inMinutes}m screen time.'),
                          duration: const Duration(seconds: 2),
                        ),
                      );
                    } else {
                       // If task was un-marked (was completed before)
                       // Optionally, deduct time back? Spec doesn't say, so for now, no deduction on un-completing.
                       // Or, prevent un-completing if time already used? Too complex for now.
                    }
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(
